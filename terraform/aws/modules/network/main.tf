...
locals {
  ...
  newbits = 32 - var.subnet_size

  # netcount is the number of subnets that we need, which is 6 in this case
  netcount = var.public_subnets_count+ var.private_subnets_count
  ...
  public_subnets  = slice(local.all_subnets, 0, var.public_subnets_count)
  private_subnets = slice(local.all_subnets, var.public_subnets_count, local.netcount)
}
